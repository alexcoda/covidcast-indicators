...
#' @importFrom vroom vroom col_character col_integer col_double col_skip col_datetime
...
load_response_one <- function(input_filename, params, contingency_run) {
  ...
  # timezone entry.
  meta_data <- vroom(full_path, skip = 2L, n_max = 1L, col_names = FALSE)$X1
  ...
  ## Sometimes the "Other" columns (*_TEXT) are read as logicals or
  ## multiple-choice columns, with comma-separated fields showing which of
  ## multiple options a user selected, as numeric, so read columns as character
  ## by default.
  input_data <- vroom(full_path, skip = 3L, col_names = col_names,
                         col_types = list(
